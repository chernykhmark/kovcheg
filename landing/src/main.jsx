...
import './styles.css'
import './price-countdown.css'
import './ticket-promo.css'
...
function Countdown({ label, labels }) {
  const [left, setLeft] = useState(() => Math.max(0, new Date(eventConfig.priceIncreaseDate) - Date.now()))
  useEffect(() => { const timer = setInterval(() => setLeft(Math.max(0, new Date(eventConfig.priceIncreaseDate) - Date.now())), 1000); return () => clearInterval(timer) }, [])
  ...
  return <div className="price-countdown"><p>{label}</p><div className="countdown" aria-label={label}>{values.map((value, index) => <div className="countdown-unit" key={labels[index]}><b>{value}</b><span>{labels[index]}</span></div>)}</div></div>
}
...
function SocialLinks({ className = '', iconSize = 15 }) { return <div className={`social-links ${className}`}>{eventConfig.socials.map((social) => <a key={social.label} href={social.url} target="_blank" rel="noreferrer">{social.icon === 'telegram' ? <Send size={iconSize} /> : <MessageCircle size={iconSize} />}{social.label}</a>)}</div> }

function Header({ lang, setLang, t }) { return <header className="header"><a href="#top" className="brand" aria-label="To top">NYR<span>27</span></a><nav>{Object.entries(t.nav).map(([id, name]) => <a href={`#${id}`} key={id}>{name}</a>)}</nav><span className="age-badge" aria-label="18 and over">18+</span><div className="language" aria-label="Language">{['ru', 'en'].map((value) => <button key={value} onClick={() => setLang(value)} className={lang === value ? 'active' : ''}>{value}</button>)}</div></header> }
...
function App() {
  ...
  return <><CanvasBackground /><div className="page" id="top"><Header {...{ lang, setLang, t }} />
    <main>
      <section id="tickets" className="hero section-shell"><motion.div initial="hidden" animate="show" transition={{ staggerChildren: .11 }} className="hero-content"><motion.p variants={reveal} className="eyebrow">{t.heroKicker}</motion.p><motion.h1 variants={reveal}>ПОСЛЕДНИЙ <span className="hero-title-tail">КОВЧЕГ <span className="hero-year">2027</span></span></motion.h1><motion.p variants={reveal} className="event-meta">{eventConfig.displayDate} <i /> {eventConfig.venue}</motion.p><motion.p variants={reveal} className="hero-sub">{t.heroSub}</motion.p><motion.div variants={reveal} className="ticket-promo hero-ticket-promo"><div className="ticket-promo-box"><p>{t.ticketPrice}</p><strong>{t.ticketDeadline}</strong><span>{t.ticketWarning}</span></div></motion.div><motion.div variants={reveal}><Countdown label={t.priceCountdown} labels={t.timerLabels} /></motion.div><motion.div variants={reveal}><BuyButton label={t.buy} /></motion.div></motion.div><a href="#tickets" className="scroll-cue"><span>{t.scroll}</span><ChevronDown size={20} /></a></section>
      ...
      <section id="program" className="section-shell section program-section"><motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: .2 }} transition={{ duration: .45 }}><motion.div variants={reveal} className="notice-social-row"><div className="social-heading"><Sparkles size={27} /><strong>{t.socialJoin}</strong></div><SocialLinks className="notice-socials" iconSize={23} /></motion.div><motion.p variants={reveal} className="notice-card newcomer-card">{t.newcomerNotice}</motion.p><p className="eyebrow">{t.programEyebrow}</p><h2>{t.programTitle}</h2><div className="timeline">{program.map((item) => <motion.div variants={reveal} className="timeline-item" key={item.time}><time>{item.time}</time><span className="timeline-dot" /><p>{item.event[lang]}</p></motion.div>)}</div><motion.p variants={reveal} className="notice-card kids-room">{t.kidsRoomNotice}</motion.p><BuyButton label={t.buy} className="program-button" /></motion.div></section>
      <section id="lineup" className="section-shell section"><motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: .12 }} transition={{ duration: .45 }}><p className="eyebrow">{t.lineupEyebrow}</p><h2>{t.lineupTitle}</h2><div className="artist-grid">{artists.map((artist) => <motion.article variants={reveal} key={artist.alias} className={`artist-card ${artist.accent}`} whileHover={reduced ? {} : { y: -6 }}><div className="art"><div className="orb" /> <div className="equalizer">{Array.from({ length: 9 }, (_, i) => <i key={i} style={{ '--delay': `${i * .1}s`, '--height': `${28 + ((i * 13) % 55)}%` }} />)}</div></div><div className="artist-info"><span>{artist.time}</span><h3>{artist.alias}</h3><p>{artist.genre}</p></div></motion.article>)}</div></motion.div></section>
    ...
    </main><footer><p>© 2026 {eventConfig.name}. {t.copyright}</p><SocialLinks /><p className="admission-rules">{t.admissionRules}</p></footer><div className="mobile-buy"><BuyButton label={t.buyShort} /></div></div></>
}
