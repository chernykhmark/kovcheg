...
function App() {
  ...
  return <><CanvasBackground /><div className="page" id="top"><Header {...{ lang, setLang, t }} />
    <main>
      <section className="hero section-shell"><motion.div initial="hidden" animate="show" transition={{ staggerChildren: .11 }} className="hero-content"><motion.p variants={reveal} className="eyebrow">{t.heroKicker}</motion.p><motion.h1 variants={reveal}>{eventConfig.name}</motion.h1><motion.p variants={reveal} className="event-meta">{eventConfig.displayDate} <i /> {eventConfig.venue}</motion.p><motion.p variants={reveal} className="hero-sub">{t.heroSub}</motion.p><motion.div variants={reveal}><Countdown labels={t.timerLabels} /></motion.div><motion.div variants={reveal}><BuyButton label={t.buy} /></motion.div></motion.div><a href="#tickets" className="scroll-cue"><span>{t.scroll}</span><ChevronDown size={20} /></a></section>
      <section id="tickets" className="section-shell ticket-promo"><motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: .45 }} transition={{ duration: .45 }} variants={reveal} className="ticket-promo-box"><p>{t.ticketPrice}</p><strong>{t.ticketDeadline}</strong><span>{t.ticketWarning}</span></motion.div></section>
