...
import { ChevronDown, Clock3, Disc3, Instagram, Music2, Send, Sparkles, Volume2 } from 'lucide-react'
...
function App() {
  ...
  return <><CanvasBackground /><div className="page" id="top"><Header {...{ lang, setLang, t }} />
    <main>
      ...
      <section id="program" className="section-shell section"><motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: .2 }} transition={{ duration: .45 }}><p className="eyebrow">{t.programEyebrow}</p><h2>{t.programTitle}</h2><div className="timeline">{program.map((item) => <motion.div variants={reveal} className="timeline-item" key={item.time}><time>{item.time}</time><span className="timeline-dot" /><p>{item.event[lang]}</p></motion.div>)}</div><BuyButton label={t.buy} className="program-button" /></motion.div></section>
    ...
    </main><footer><p>© 2026 {eventConfig.name}. {t.copyright}</p><div>{eventConfig.socials.map((social, i) => <a key={social.label} href={social.url} target="_blank" rel="noreferrer">{i === 0 ? <Send size={15} /> : <Instagram size={15} />}{social.label}</a>)}</div></footer><div className="mobile-buy"><BuyButton label={t.buyShort} /></div></div></>
}
