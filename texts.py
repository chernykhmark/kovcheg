...
WELCOME = (
    "Привет! Это бот для покупки билетов на мероприятие.\n\n"
    "Выберите действие в меню ниже."
)

# Подписи пунктов клиентского меню (reply)
BTN_BUY = "🎫 Купить билет"
BTN_EVENT_PAGE = "✨ Открыть афишу"
...
FAQ_ITEMS = [
    ("Где будет проходить мероприятие?", "Адрес указан на афише мероприятия."),
    ("Как оплатить билет?", "После оформления заказа бот пришлет реквизиты для оплаты по СБП."),
...
def order_created(ticket_type_name: str, quantity: int,
    ...
    return (
        ...
        f"затем пришлите скриншот оплаты в этот чат."
    )
